...
2. **動画を読み込む** を押します。
3. プロジェクト名、チームA名、チームB名、各チームの背番号リストを設定します。
4. チームAまたはチームBを選びます。
5. 背番号をタップします。
6. 再生中の時刻で **サーブをタグ** または **スパイクをタグ** を押します。
7. タグ一覧で時刻、チーム、プレー種別、背番号、メモを編集できます。
8. プレー種別、チーム、背番号で絞り込めます。
9. タグの何秒前から何秒後まで再生するかを選びます。
10. **絞り込みタグを連続再生** で、該当タグを順番に再生します。
11. 必要に応じてJSONを書き出し、あとで読み込んで作業を続けます。
12. タグ一覧はCSVとして書き出せます。
...
画面表示は日本語ですが、JSON内の `play` は互換性のため `serve` / `spike` のまま保存します。
